...
{
    public abstract class SelectableTracksListViewModel : ReactiveScreen // ReactiveObject
    {
        ...
        protected SelectableTracksListViewModel() { }

        public SelectableTracksListViewModel(
            ...
            Func<Track, TrackViewModel> trackViewModelFactoryMethod)
        {
            ...
            this._readLibraryService.Tracks.Connect().OnItemRemoved(c => Debug.WriteLine($"TracksVM.Tracks.OnItemRemoved: {c.Location}"));
            ...
            this.PlayAll = ReactiveCommand.CreateFromTask(
                async () =>
                {
                    ...
                    var uris = this._readLibraryService.Tracks.Connect().Transform(track => track.Location).AsObservableList();
