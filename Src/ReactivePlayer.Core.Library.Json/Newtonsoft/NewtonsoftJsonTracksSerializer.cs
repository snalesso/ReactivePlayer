...
namespace ReactivePlayer.Core.Library.Json.Serializers
{
    public sealed class NewtonsoftJsonTracksSerializer : EntitySerializer<Track, uint>, IDisposable
    {
        ...
        private const string DBFileName = nameof(Tracks) + ".json";
        ...
        protected override async Task DeserializeCore()
        {
            ...
            try
            {
                ...
                if (this._entities == null)
                {
                    ...
                    var deserializedTracksCollection = JsonConvert.DeserializeObject<IEnumerable<Track>>(dbContentAsString, new NewtonsoftJsonTrackConverter()) ?? Enumerable.Empty<Track>();
        ...
        protected override async Task SerializeCore()
        {
            ...
            try
            {
                ...
                using (var sw = new StreamWriter(this._dbFileStream, this._encoding, NewtonsoftJsonTracksSerializer.RWBufferSize, true))
                {
                    ...
                    await sw.FlushAsync();
                    this._dbFileStream.SetLength(this._dbFileStream.Position);
                }
            }
            catch (Exception ex)
            {
