...
{
    public sealed class NewtonsoftJsonPlaylistsSerializer : EntitySerializer<PlaylistBase, uint>, IDisposable
    {
        #region constants & fields

        private const string DBFileName = nameof(Playlists) + ".json";
        ...
        public NewtonsoftJsonPlaylistsSerializer()
        ...
        protected override async Task DeserializeCore()
        {
            ...
            try
            {
                ...
                if (this._entities == null)
                {
                    ...
                    using (var sr = new StreamReader(this._dbFileStream, this._encoding, true, NewtonsoftJsonPlaylistsSerializer.RWBufferSize, true))
                    {
                        sr.BaseStream.Position = 0;
                        dbContentAsString = await sr.ReadToEndAsync();//.ConfigureAwait(false);
                    }

                    var deserializedTracksCollection = JsonConvert.DeserializeObject<IEnumerable<PlaylistBase>>(dbContentAsString, new NewtonsoftJsonPlaylistBaseConverter()) ?? Enumerable.Empty<PlaylistBase>();
                    var kvps = deserializedTracksCollection.Select(t => new KeyValuePair<uint, PlaylistBase>(t.Id, t));
                    this._entities = new ConcurrentDictionary<uint, PlaylistBase>(kvps);
        ...
        protected override async Task SerializeCore()
        {
            ...
            try
            {
                ...
                using (var sw = new StreamWriter(this._dbFileStream, this._encoding, NewtonsoftJsonPlaylistsSerializer.RWBufferSize, true))
                {
